...
# 30
#想法 
为什么我们的logic->logic都要走router转发，既然都是全连接网状拓扑，为什么不直接logic和logic直连，例如一些不动的对象，例如callService，其实不需要走router吧。是因为router本来就是要全连接logic的，所以不需要logic再自己互相连了？多一次内网转发消耗不大？然后对于有迁移属性的对象来说需要一个不动点，用的就是router，那这样处理就可以比较统一？
目前想到
一个是可以减少连接，logic与logic之间的全连接不需要
一个是有会迁移的对象，为了统一处理（迁移不迁移的对象都能支持），走router进行一次转发