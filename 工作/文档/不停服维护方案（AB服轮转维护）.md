...
## 数据一致性的设计
### 互斥的思想非常清晰
通过对象来进行分析和互斥，mmo游戏99%的业务逻辑都集中在3类对象中，AvatarActor，Space，Service。因为复杂的一个游戏想要不遗漏其实是挺难得，那么通过某个维度去进行地毯式的分析会比较完备，不容易遗漏。
其中每个Space都是相互独立的，是天然支持不停服维护的。
AvatarActor则通过登录的互斥来做到互斥。另外需要定义好数据的所有者，例如玩家所属帮会id，这个看起来也是玩家的数据，但是数据的所有权在GuildService，此类数据在玩家登录期间会有获取的机制，在玩家侧只是一个缓存，不会以玩家身上的数据为准。
Service相对比较复杂，情况较多，有些有数据，有些没有，有些有共享数据，有些没有。
### 对于不同的数据进行分级
对于没有存盘数据的Service，基本可以认为是天然支持轮转的，可以在新老服同时启动服务自己的服务器。例如OnlineService，RouterService
对于有存盘数据的Service如果存盘数据互不相关，也是天然支持轮转的，例如副本的roll点记录。
如果有存盘数据且会共享数据的Service，优先建议屏蔽，例如FriendService，老服在2阶段之后就不支持增加好友，删除好友之类的操作。没法屏蔽的（本身数据上就是需要的，或者策划要求的）目前的方案是缓存加转发，统一由新服的Service去处理。这个方案会有少量时间的延迟，针对可能的低延迟的需求，也设计了2个方案（新老服同时处理和压缩阶段2,3时间，通过增量存盘来实现），不过目前不需要。
## 阶段的设计边界清晰，每个阶段有回滚机制
每个阶段做的事情非常清晰，而且出现问题如何处理也有明确的设计
## 工程上的通用化设计
不停服维护对于玩家来说体验很好，但是也会相应的增加开发的复杂度。
对于玩家登录互斥，一次性的修改后后续无需额外关注。
主要的开销在于Service的开发，所以我前面给出了Service的处理意见，如何判断Service是否需要对不停服维护，需要的话怎么做，对于屏蔽增加了通用的拦截处理。
