...
## 一些个人理解和重要信息
### 增量与全量
logic -> dbmgr 是增量更新的，dbmgr -> mongo是全量的（增量也做了功能，但是默认不开，realtime_save_incremental_update_threshold），引擎最后用的replace_one接口
```
$ grep realtime_save_incremental_update_threshold *.log
dbmgr.log:2026-02-25 16:09:33,167346 - dbmgr - 60328 - INFO - [doraemon] common/utils/options.hpp:97 - [options.realtime_save_incremental_update_threshold]=0 init python
```
有开关，目前默认为0，就是存盘操作是全量的
logic要求dbmgr立刻存盘，也是只发元数据的，例如_id，dbmgr上有一份完整的数据，落地就好。
然后这份完整的entity数据会按照一级属性进行划分，每次存盘序列化，只需要重新序列化脏数据（以一级属性为单位），提升性能。(由于存储的是序列化好的数据，内存开销不会太大，盲猜只有entity_cache本身的1/2以下，所以已经改为默认开启)
```
$ grep enable_realtime_save_cache dbmgr.log
2026-02-25 16:09:33,167307 - dbmgr - 60328 - INFO - [doraemon] common/utils/options.hpp:97 - [options.enable_realtime_save_cache]=true init python
```
### 同步间隔
realtime_save_sync_props_batch_size(作用于logic)，默认100。意思是每个entity在一个tick内积累了100次同步属性的请求之后才把这些请求合批打包发送给dbmgr进行处理。在每个tick结束后(大概1秒10次)、发起存盘之前和迁移之前，也会把当前积累的属性同步请求发送给dbmgr进行处理。这个值太小了影响性能，太大了可能导致卡顿甚至一个包超大，默认100就蛮好
```
$ grep realtime_save_sync_props_batch_size *.log
logic_0.log:2026-02-25 16:09:32,966845 - logic_0 - 122048 - INFO - [doraemon] common/utils/options.hpp:97 - [options.realtime_save_sync_props_batch_size]=100 init lua
```
一个tick内一般也不会超过100我估计，反正有修改最慢就是一次tick都会存一次
### 容灾
在dbmgr挂了之后，logic会收到通知，绑定在这个dbmgr的entity需要重新序列化数据，发给新绑定的dbmgr。由于担心一瞬间太多entity需要做这个序列化操作会使logic卡顿，会从0~realtime_save_choose_new_dbmgr_max_delay之间random一个时间，在这个时间之后再进行此操作。
```
$ grep realtime_save_choose_new_dbmgr_max_delay *log
logic_0.log:2026-02-25 16:09:32,966921 - logic_0 - 122048 - INFO - [doraemon] common/utils/options.hpp:97 - [options.realtime_save_choose_new_dbmgr_max_delay]=5 init lua
```
在选择dbmgr时，logic会选择和他不同机器的dbmgr，当迁移发生时，如果迁移后的logic和entity之前的dbmgr在同一个机器，会触发切换的流程，进行entity的存盘，然后在新的dbmgr上加载。如果本来就是不同的机器，不会触发切换。也就是说logic应该和所有的dbmgr都有连接，只是默认的情况下可能会用配置的prefer_dbmgr。看这段
`同时这个entity会记录下这个dbmgr的pid，也就是绑定，在迁移时也会保持这个绑定关系不变。后面这个entity都只会通过这个绑定了的dbmgr进行存盘。`
另外存盘目前看dbmgr没有定时的落地，只有脚本层让它存才会存，脚本层会有定时的落地请求发到dbmgr（应该是15分钟），这个就完全给业务层控制了。
另外看起来增量的存盘是有ack机制的，可以看[[Entity Term]] 
存盘请求必须要等到DB进程确认回复之后才能将其从请求队列中移除，然后再发送可能存在的待发送请求（这对实现纯增量存盘很重要）